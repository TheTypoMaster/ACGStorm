...
<div class='' >
        ...
        <div class="all_dingdan">
          <ul class="dingdan_list">
            <li><a href="<?php echo $message_url; ?>">给我评论
              </a></li>
            <li><a href="<?php echo $reply_url; ?>">给我回复       
       </a></li>
            <li><a href="<?php echo $comment_url; ?>"  class="on" >我发表的评论
                   </a></li>
          </ul>

          <div class="dg_dingdan">
            <div style="padding:10px 20px;font-size:14px;border-bottom:1px #e5e5e5 solid">Hi，<?php echo $firstname; ?>，您发表过 <span style="color:#ed5564;font-weight:bold"><?php echo $Mycomment_total; ?></span> 条评论，认真的点评都有可能被推荐为精彩评论哦！</div>
           
             <?php foreach($Mycomment_info as $Mycomment) {  ?>
            <div class="comment-list" id="com_<?php echo $Mycomment['comment_id'];?>">
                  <div class="com_box">
                  <div class="com_img">                  
                  <?php if($Mycomment['face']) {  ?>
                  <img src="<?php echo $Mycomment['face']; ?>"/></div>
                  <?php }else{  ?>
                  <img src="uploads/big/0b4a96400b2372d25da769647bfe4059.jpg"/></div>
                  <?php } ?>                  
                  <div class="com_out_line" ></div>
                  <div class="com_text">
                      <div class="rp_title">对<span class="re_user_name" style="color:#0c73c2;font-size: 14px;"><?php echo $Mycomment['firstname']; ?></span><span> 的<?php if(1 == $Mycomment['message_flag']) { ?>晒单<?php }else if(2 == $Mycomment['message_flag']){ ?>心情<?php } ?> </span><span style="color:#0c73c2;font-size: 14px;" class="comment_content">“<?php if(strlen($Mycomment['message_text']>108)) {echo substr_replace($Mycomment['message_text'],"..." ,108); }else{  echo $Mycomment['message_text']; }?>”</span><span> 发表了评论。 </span>
                          <p class="comment_content"><?php echo $Mycomment['comment_text']; ?></p>
                      </div>
                          <div style="margin-top:14px;color:#a0a0a0"><span><?php echo date("Y-m-d H:i:s",$Mycomment['addtime']);?></span><span style="float:right"><a class="del_cmt" id="<?php echo $Mycomment['comment_id'];?>" style="color:#0c73c2">删除</a></span></div>
                  </div>
