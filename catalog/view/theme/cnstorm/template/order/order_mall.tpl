    ...
    <div class="ui-box-body">
      <div class="ui-form-item">
        <div class="ui-form-control">
          <label>特别关注 :</label>
          <a <?php if (!isset($pass_order_status_id)) echo "class='dingdan_active'" ?> href="order-order-mall.html&order_status_id=0" ><?php echo $text_all_order; ?><font color="#FD522E">( <?php echo $num_mall; ?> )</font></a>
          <?php foreach ($order_statuses as $order_status) { if (in_array($order_status['order_status_id'],array(2,4,6,8))) {?>
          <a <?php if (isset($pass_order_status_id) &&  $pass_order_status_id == $order_status['order_status_id'])echo "class='dingdan_active'" ?> href="order-order-mall.html&order_status_id=<?php echo $order_status['order_status_id']; ?>" ><?php echo $order_status['name']; ?> <font color="#FD522E">(
...
<?php } } ?>
      ...
      <div class="ui-form-item">
        <div class="ui-form-control">
          ...
          <a <?php if (isset($pass_order_status_id) &&  $pass_order_status_id == $order_status['order_status_id'])echo "class='dingdan_active'" ?> href="order-order-mall.html&order_status_id=<?php echo $order_status['order_status_id']; ?>" ><?php echo $order_status['name']; ?> <font color="#FD522E">( <?php echo $order_status['total']; ?> )</font></a>
          <?php }} ?>
        </div>
      </div>
    </div>
  ...
  <div class="order_list">
    <ul class="detail_dd">
      ...
      <li class="detail_fi">
        <select onchange='search_change_mall(this.options[this.options.selectedIndex].value)' style="margin-top:5px;" name="filter_order_status_id" id="filter_order_status_id">
...
function searchOrderForTime(){
    ...
    if (!re.test(startTime) || !re.test(endTime)){
        ...
        window.location="order-order-mall.html&st="+startTime+"&et="+endTime;
    }else{
...
function searchOrderForKeyWord(){
    ...
    if (searchKeyWord.trim('') != ''){
        if (si == 1){
            window.location="order-order-mall.html&order_id="+searchKeyWord;
        }else if(si == 2){
            window.location="order-order-mall.html&sk="+searchKeyWord;
        }
    }else{
...
function modify(id){
    $.ajax({
      url:'index.php?route=order/order/mall',
      dataType:"json",
