...
class ModelCatalogProduct extends Model {
    ...
    public function deleteProductCart($product_id) {
        $this->db->query("DELETE FROM " . DB_PREFIX . "cart WHERE cart_id = '" . (int) $product_id . "'");
    }
    ...
    public function getProduct($product_id) {
        ...
        if ($query->num_rows) {
            return array(
                ...
                'api_json' => array_key_exists('api_json', $query->row) ? $query->row['api_json'] : ""
            );
        } else {
            return false;
        }
...
//guanzhiqiang 20150821 屏蔽情趣用品
        $sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id)
                  LEFT JOIN (SELECT product_id,MIN(category_id) category_id FROM oc_product_to_category GROUP BY product_id) pc ON(p.`product_id`=pc.product_id) 
                  WHERE pd.language_id = '" . (int) $this->config->get('config_language_id') . "' AND pc.category_id!=193 AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int) $this->config->get('config_store_id') . "'";
    ...
    //大家最爱
    public function getLoveProducts($category, $limit) {
        $category_id = $this->db->escape(implode(",", $category));
        $limit = (int) $limit;
        $sql = <<<EOF
                SELECT pc.category_id,p.product_id,p.image,p.price,pd.name FROM oc_product_to_category pc
LEFT JOIN oc_product p ON(p.product_id=pc.product_id) 
LEFT JOIN oc_product_description pd ON (p.product_id = pd.product_id) 
WHERE pc.category_id IN($category_id) AND  pd.language_id = '2' ORDER BY pc.product_id DESC LIMIT $limit
EOF;
    ...
    //获取轮播图
    public function getLunboPics($type = 1, $order = 1) {
        if (2 == $type) {
            if (1 == $order)
                $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "lunbo_pic WHERE `type` = '" . (int) $type . "' AND `flag`=0 ORDER BY `sort` limit 3");
            else
                $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "lunbo_pic WHERE `type` = '" . (int) $type . "' AND `flag`=0 ORDER BY `sort` DESC limit 3");
        }else {
    ...
    //seo描述信息
    public function getMetaInfo($product_id) {
        $sql = "SELECT * FROM " . DB_PREFIX . "cosplay_product_description ";
        ...
        if (empty($query->rows[1])) {
            return $result;
        }
        return $query->rows[1];
    }

    public function getShopstrategy($limit) {
        $sql = "select *,UNIX_TIMESTAMP(endtime) - UNIX_TIMESTAMP(starttime) as timeca from oc_favourable where now() > starttime and now()< endtime  order by sort asc limit " . $limit . "";
        $rows = $this->db->query($sql);
        return $rows->rows;
    }

    public function getShopstrategys() {
        $sql = "select *,UNIX_TIMESTAMP(endtime) - UNIX_TIMESTAMP(starttime)as timeca from oc_favourable where now() > starttime and now()< endtime ";
        $rows = $this->db->query($sql);
