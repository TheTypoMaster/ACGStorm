...
class ControllerAccountPassword extends Controller {
	...
	public function index() {
		...
		$this->data['breadcrumbs'][] = array(
			'text'      => $this->language->get('text_editpassword'),
	...
	protected function validate() {
       ...
       if ((utf8_strlen($this->request->post['oldpassword']) < 4) || (utf8_strlen($this->request->post['oldpassword']) > 20)) 
       {
			$this->error['oldpassword'] = $this->language->get('error_oldpassword');
       }
       else
       {
            $customer_info = $this->model_account_customer->getCustomer($this->session->data['customer_id']);
            
            if($customer_info['password'] != md5($this->request->post['oldpassword']) && $customer_info['password'] != (sha1($customer_info['salt'] . sha1($customer_info['salt'] . sha1($this->request->post['oldpassword'])))))
            {
                $this->error['oldpassword'] = $this->language->get('error_oldpassword');
            }
