...
//echo "<div style='display:hidden'>".$uname ."-".$uid."</div>";
            ...
            if (count($row->row) == "0") {
                ...
                if (empty($row2->row['customer_id'])) {//customer_id查询为空时,说明是weibo初次登录，需完善注册信息  guanzhiqiang 20150601
                    $this->session->data['from'] = 'weibo';
                    $this->session->data['nick'] = $uname;
                    $this->session->data['oauthuid'] = $uid;
    ...
    public function login_qq() {
        ...
        if ($code) {
            ...
            $row = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE `from` = 'qq' AND (oauthuid= '" . $user->openid . "' OR firstname = '" . $uname . "' OR tname= '" . $uname . "')"); //SQL 范围 FROM=QQ  QQ名改后，就无法登录，需重新注册 
            ...
            if (count($row->row) == "0") {
                ...
                if ($row->row['oauthuid'] == "") {
                    $this->db->query("UPDATE " . DB_PREFIX . "customer SET oauthuid= '" . $user->openid . "' WHERE `from` = 'qq' AND firstname = '" . $uname . "'  LIMIT 1");
                }
                unset($this->session->data['guest']);
                ...
                if (!empty($user->openid)) {
                    $this->db->query("UPDATE " . DB_PREFIX . "customer SET logintime = UNIX_TIMESTAMP(NOW()) WHERE `from` = 'qq' AND oauthuid= '" . $user->openid . "' AND oauthuid!='' LIMIT 1");
                }
