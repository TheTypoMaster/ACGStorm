 ...
 * *****************************************************************************************
 */
class Controllerproductsnatch extends Controller {
    ...
    public function index() {
        ...
        $this->data['title'] = "我要代购";
        
        $this->data['keywords'] = "我要代购";
        
        $this->data['description'] = "我要代购";
    	...
    	if (file_exists ( DIR_TEMPLATE . $this->config->get ( 'config_template' ) . '/template/product/snatch.tpl' )) {
    	   
			$this->template = $this->config->get ( 'config_template' ) . '/template/product/snatch.tpl';
		} else {
		  
			$this->template = 'default/template/product/snatch.tpl';
		}
    ...
    public function iteminfo() {  
    	...
    	if (false !== strpos ( $search, 'taobao.com' ) || false !== strpos ( $search, 'tmall.com' ) || false != strpos($search, 'yao.95095')) {
    		...
    		if ($flag) {
    			$this->data['prifex'] = 'TB';
    			$this->data['search'] = isset($this->data['num_iid'])?'https://item.taobao.com/item.htm?id=' . $this->data['num_iid']:'';
    			...
    			// var_dump($this->data);
    			if (file_exists ( DIR_TEMPLATE . $this->config->get ( 'config_template' ) . '/template/product/snatch_info.tpl' )) {
    				$this->template = $this->config->get ( 'config_template' ) . '/template/product/snatch_info.tpl';
    			} else {
    				$this->template = 'default/template/product/snatch_info.tpl';
    			}
			...
			$date [7] ='';
    		// 商铺名称
    		if (preg_match ( '#<a[^>]*class="has-tips"[^>]*href="(.*)"[^>]*><span class="(info |info  gd-info )">(.*)<\/span><\/a>#isU', $html, $matches )) {
    			if (! empty ( $matches [1] )) {
			...
			if($date [7]=='' || $date [8]==''){
					if (preg_match ( '/<meta property="og:product:nick" content="name=(.*); url= (.*)\"\/>/isU', $html, $matches )) {
					if (! empty ( $matches [2] )){
							$date [8] =$matches [2];
						}
    	...
    	//	echo $this->data ['isbn'];
    		...
    		// 商品价格
    		if ( preg_match('/#a-size-medium a-color-price inlineBlock-display offer-price a-text-normal price3P\"[^>]*>(.*)<\/span>#isU/', $html, $matches ) || preg_match ( '#id=\"priceblock_ourprice\"[^>]*>(.*)<\/span>#isU', $html, $matches ) || preg_match ( '#priceLarge\"[^>]*>(.*)<\/b>#isU', $html, $matches ) || preg_match ( '#id=\"listPriceValue\"[^>]*>(.*)<\/span>#isU', $html, $matches ) || preg_match ( '#a-size-medium a-color-price offer-price a-text-normal\"[^>]*>(.*)<\/span>#isU', $html, $matches ) || preg_match ( '#id=\"priceblock_saleprice\"[^>]*>(.*)<\/span>#isU', $html, $matches )) {
