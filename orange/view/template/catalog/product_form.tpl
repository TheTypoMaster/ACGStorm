...
<div id="content">
  ...
  <div class="box">
    ...
    <div class="content">
      ...
      <a href="#tab-onekey"><?php echo $tab_onekey; ?></a>
      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        <div id="tab-onekey">
		    ...
		    <tr>
                ...
                <td><input type="text" id="entry_url_get" name="entry_url_value" size="80" value="<?php echo $entry_url_value; ?>" /></td>
                <td><textarea id="api_json" name="api_json" style="width: 500px; height: 50px;"><?php echo $api_json;?></textarea></td>
				<td><a class="button" onclick="getgoodinfo()"><?php echo $entry_url_get;?></a></td>
			</tr> 
            </table>
        </div>
          ...
          <div id="language<?php echo $language['language_id']; ?>">
            <table class="form">
              <tr>
                ...
                <td><input type="text" id="product_description[<?php echo $language['language_id']; ?>][name]"  name="product_description[<?php echo $language['language_id']; ?>][name]" size="100" value="<?php echo isset($product_description[$language['language_id']]) ? $product_description[$language['language_id']]['name'] : ''; ?>" />
        ...
        <div id="tab-data">
          <table class="form"> 
            <tr>
              ...
              <td><input type="text" name="model" size="80" value="<?php echo $model; ?>" /></td>
            </tr>
            <tr>
              <td><?php echo $entry_sku; ?></td>
              <td><input type="text" name="sku" size="80" value="<?php echo $sku; ?>" /></td>
            </tr>
            <tr>
              <td><?php echo $entry_upc; ?></td>
              <td><input type="text" name="upc" size="80" value="<?php echo $upc; ?>" /></td>
            </tr>
            <tr>
              <td><?php echo $entry_ean; ?></td>
              <td><input type="text" name="ean" size="80" value="<?php echo $ean; ?>" /></td>
            </tr>
            <tr>
              <td><?php echo $entry_jan; ?></td>
              <td><input type="text" name="jan" size="80" value="<?php echo $jan; ?>" /></td>
            </tr>
            <tr>
              <td><?php echo $entry_isbn; ?></td>
              <td><input type="text" name="isbn" size="80" value="<?php echo $isbn; ?>" /></td>
            </tr>
            <tr>
              <td><?php echo $entry_mpn; ?></td>
              <td><input type="text" name="mpn" size="80" value="<?php echo $mpn; ?>" /></td>
            </tr>
            <tr>
              <td><?php echo $entry_mpnurl; ?></td>
              <td><input type="text" name="mpnurl" size="80" value="<?php echo $mpnurl; ?>" /></td>
            </tr>
            <tr>
              <td><?php echo $entry_num_iid; ?></td>
              <td><input type="text" name="num_iid" size="80" value="<?php echo $num_iid; ?>" /></td>
            </tr>
            <tr>
              <td><?php echo $entry_location; ?></td>
              <td><input type="text" name="location" size="80" value="<?php echo $location; ?>" /></td>
            </tr>
            <tr>
                <td><?php echo $entry_price; ?></td>
                <td><input type="text"  name="price" size="80" value="<?php echo $price; ?>" /><span class="required">(<?php echo $price_warn; ?>)</span></td>
            </tr>
            <tr>
        ...
        <div id="tab-links">
          <table class="form">
            <tr>
              <td>&nbsp;&nbsp;是否显示在推荐购首页：</td>
              <td>
                <input type="radio" name="frontpage" value="1" <?php if ($frontpage) echo('checked="checked"'); ?> /><?php echo $text_yes; ?>
                <input type="radio" name="frontpage" value="0" <?php if (!$frontpage || 0==$frontpage) echo('checked="checked"'); ?> /><?php echo $text_no; ?>
              </td>
            </tr>
            <tr>
...
function getgoodinfo() {		
		var d = $.trim(document.getElementById("entry_url_get").value);
		//alert(d);
        if (d.length <= 0 || d == "http://") {
            alert("<?php echo $empty_entry_url?>");
            return false;
        }
        ...
        if (!i.test(d)) {
            alert("<?php echo $error_entry_url?>");
            return false;
        }
	
		$.ajax({
		        type: "POST",
                url: 'index.php?route=catalog/product/getinfo&token=<?php echo $token; ?>',
                dataType: "json",
				data: "entry_url=" +  encodeURIComponent(d),
				...
				success: function(data){  
                                          ...
                                          { 
                                            if(data['flag'])
                                            {
                                               $('input[name=\'product_description[2][name]\']').attr("value",data['good_name']);
                                               $('input[name=\'price\']').attr("value",data['good_price']);
                                               $('input[name=\'num_iid\']').attr("value",data['num_iid']);
                                               $('input[name=\'location\']').attr("value",data['good_url']);
                                               $('input[name=\'quantity\']').attr("value",data['good_quantity']);
                                               $('input[name=\'model\']').attr("value",data['store_name']);
                                               $('input[name=\'sku\']').attr("value",data['seller_url']);
                                               $('input[name=\'upc\']').attr("value",data['seller_name']);
                                               $('input[name=\'ean\']').attr("value",data['good_color']);
                                               $('input[name=\'jan\']').attr("value",data['good_size']);
                                               $('input[name=\'isbn\']').attr("value",data['good_fee']);
                                               $('input[name=\'mpn\']').attr("value",data['storename']);
                                               $('input[name=\'mpnurl\']').attr("value",data['store_url']);
                                               $('#thumb').attr('src', data['good_img']); 
                                               var good_img=data['good_img'];
                                               var url=good_img.substr(good_img.indexOf("data"),good_img.indexOf("-222x222.jpg")-good_img.indexOf("data"))+".jpg";
                                               $('#image').attr('value', url);
                                               $("#api_json").val(data['api_json']);
                                               alert("一键获取商品信息成功，请继续完善资料！");
                                            }
                                            else
                                            {
                                                alert("抱歉，目前本系统的一键获取商品信息功能暂不支持非淘宝商品，请手工录入！");
                                            } 
                                          }
