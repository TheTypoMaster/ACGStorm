    ...
    <div class="content">
      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        <div id="tab-general">
          ...
          <div id="language<?php echo $language['language_id']; ?>">
            <table class="form">
              <tr  style="height: 130px;">
				...
				<input type="text" name="product_description[<?php echo $language['language_id']; ?>][name]" size="80" value="<?php echo isset($product_description[$language['language_id']]) ? $product_description[$language['language_id']]['name'] : ''; ?>" />
				  ...
				  <?php if($language['language_id']>1){ ?>
					...
					<div class="image" style="position: absolute;left: 700px;top: -15px;">
         ...
         <td><textarea id="editor_<?php echo $language['language_id']; ?>" name="product_description[<?php echo $language['language_id']; ?>][description]"  style="width:700px;height:300px;">
<?php echo isset($product_description[$language['language_id']]) ? $product_description[$language['language_id']]['description'] : ''; ?>
              ...
              <tr>
			   ...
			   <tr>
				 <td>首页推荐</td>
                 <td>
				 <select name="product_description[<?php echo $language['language_id']; ?>][tophome]">
				 <option value="1" <?php if($product_description[$language['language_id']]['tophome'] == 1 ){ ?> selected="selected" <?php } ?>>首页推荐</option>
				  <option value="0" <?php if($product_description[$language['language_id']]['tophome'] == 0 ){ ?> selected="selected" <?php } ?> >不推荐</option>
				 </select>
				 </td>
			  </tr>
        ...
        <div id="tab-data">
          <table class="form">
			...
			<tr>
				<td><input type="button" value='添加sku' id="addsku" /></td>
				<td><table id="newsku">
					<?php 
						if($price_size){
							foreach($price_size as $key=>$szie){
								foreach($szie as $k=>$color){
									?>
								<tr class="newsku">
								<td>颜色:<input type="text" name="color[]" value="<?php echo $k; ?>"></td>
								<td>尺寸:<input type="text" name="szie[]" value="<?php echo $key;?>"></td>
								<td>库存:<input type="text" name="kucun[]" value="<?php echo $color['kucun'] ?>"></td>
								<td>价格:<input type="text" name="prices[]" value="<?php echo $color['price'] ?>"></td>
								<td><a href="javascript:;" onclick="del(this)">删除</a></td></tr>
		...
		$(function(){
			$('#addsku').click(function(){
				var str="<tr class='newsku'><td>颜色:<input type='text' name='color[]' /></td>"
				    str+="<td>尺寸:<input type='text' name='szie[]' /></td>"
					str+="<td>库存:<input type='text' name='kucun[]'/></td>"
					str+="<td>价格:<input type='text' name='prices[]' /></td>"
...
$('input[name=\'category\']').autocomplete({
	...
	source: function(request, response) {
		$.ajax({
			url: 'index.php?route=catalog/cosplay_category/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request.term),
			dataType: 'json',
			success: function(json) {		
				response($.map(json, function(item) {
					return {
...
$('input[name=\'related\']').autocomplete({
	...
	source: function(request, response) {
		$.ajax({
			url: 'index.php?route=catalog/cosplay_product/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request.term),
			dataType: 'json',
			success: function(json) {		
				response($.map(json, function(item) {
					return {
...
//--></script>
	<script>
			$(function(){
				$('#languages a:first').removeClass('selected');
				$('#languages a:last').addClass('selected');
				$('#language1').removeClass('selected').attr('style','display: none;');
				$('#language2').addClass('selected').attr('style','display: block;');
			})
		</script>
                <script charset="utf-8" src="/keditor/kindeditor-all-min.js"></script>
...
<script>
        KindEditor.ready(function(K) {
                ...
                afterBlur: function () { this.sync(); }
                                    ...
                                    window.editor = K.create('#editor_2', {
                afterBlur: function () { this.sync(); }
            });
        });
</script>
