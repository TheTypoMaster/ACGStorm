...
class ControllerLocalisationCountry extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$area = $this->model_setting_maintain->getArea($result['areaid']);
	...
	protected function getForm() {
		...
		if (isset($this->request->post['areaid'])) {
			$this->data['selectedValue'] = $this->request->post['areaid'];
		} elseif (!empty($country_info)) {
			$this->data['selectedValue'] = $country_info['areaid'];
		} else {
			$this->data['selectedValue'] = '0';
		}
